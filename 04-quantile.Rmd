# Quantile and depth-based measures {#quantile-methods}
...
A quantile for a single variable is a point that divides the sample space into two based on the probability of observations falling below or above the quantile value. For example, if the quantile corresponding to probability 0.8 is denoted by $q_{0.8}$, and $y$ is a new observation, then $P(y \le q_{0.8}) = 0.8$ and $P(y > q_{0.8}) = 0.2$.
...
Using this idea, we can calculate some of the quantiles for Test cricket batting averages, considering only those batters who have played at least 20 innings.
...
There are $n=`r n`$ observations with a minimum of $\hat{q}_0 = y_{(1)} = `r quartiles[1]`$ and a maximum of $\hat{q}_1 = y_{(`r n`)} = `r quartiles[5]`$. Because there is an even number of observations, the sample median is the average of the middle two observations in the ordered data, which is 
$$\hat{q}_{0.5} = 0.5 (y_{(`r k[3]`)} + y_{(`r k[3]+1`)}) = `r quartiles[3]`.$$ 
The sample quartiles are given by
$$\hat{q}_{0.25} = `r 1- gamma[2]` y_{(`r k[2]`)} + `r gamma[2]` y_{(`r k[2]+1`)} = `r quartiles[2]`$$
and
$$\hat{q}_{0.75} = `r 1- gamma[4]` y_{(`r k[4]`)} + `r gamma[4]` y_{(`r k[4]+1`)} = `r quartiles[4]`.$$
The 0.99 sample quantile is given by
$$\hat{q}_{0.99} = `r 1- gamma[6]` y_{(`r k[6]`)} + `r gamma[6]` y_{(`r k[6]+1`)} = `r quartiles[6]`.
$$
...
The other sample quantiles as defined in the previous section do not correspond exactly to specific depths, but @Tukey1977-xd introduced variations for some quantiles that are based on depths. He called these "letter values".
...
We will continue to use the batting averages discussed in the previous section as an example. The depths of the first four letter values are given by
$$
  d_1 = `r d[1]`,\quad
  d_2 = `r d[2]`,\quad
...
Figure \@ref(fig:cricketbox) shows a boxplot of the cricket batting average data, previously shown in Figure  \@ref(fig:cricket1).
...
n01b$v2[20] <- 4
n01b %>% filter(barbato_anomaly(v2))
```
...
A diagram will help illustrate the idea. Suppose we have the observations shown in Figure \@ref(fig:tukeydepth). These are the first 10 observations from the first two variables of `n01`. The red line divides the plane into two sections which are called "half spaces". We are interested in the depth of the orange point (which is not one of the observations).
...
The depth region $D_k$ is the set of all points $\bm{z}$ with Tukey depth at least $k$. These form a series of nested convex hulls where $D_{k+1} \subseteq D_k$. The depth regions for our example are shown in Figure \@ref(fig:depthregions). The blue region is depth 1, the orange region is depth 2, the pink region is depth 3, and the green region is depth 4.

(ref:depthregions) The depth regions for the 10 bivariate observations from Figure \@ref(fig:tukeydepth). The depth median is shown as the yellow point in the centre. The 10 observations are shown in black. These lie at the outer edges of the depth regions.
...
A simple way to define a multivariate median is the "centre of gravity" of all points of maximum depth [@bivariatemedian]. In Figure \@ref(fig:depthregions), the points of maximum depth are shown in the central green region. The centre of gravity of this region is the yellow point in the middle.

### Convex hull peeling depths

Imagine that each of the observations shown in Figure \@ref(fig:tukeydepth) is a nail hammered into a board and we are looking at it from above. An elastic band wrapped around all observations will form a "convex hull" as shown on the left of Figure \@ref(fig:convexhull).

(ref:convexhull) Left: The convex hull containing the 10 bivariate observations from Figure \@ref(fig:tukeydepth). Right: The second convex layer formed with the same points.

```{r convexhull, fig.cap="(ref:convexhull)", echo=FALSE}
...
This is the first convex layer. If the points on the perimeter are removed, and the convex hull of the remaining points is constructed, we obtain the second convex layer, shown on the right of Figure \@ref(fig:convexhull). The third convex layer will consist of the one remaining point that is within the second convex hull.
...
In this example, the convex hulls shown in Figure \@ref(fig:convexhull) are equivalent to the depth regions shown in Figure \@ref(fig:depthregions). But that will not always be true.
...
Figure \@ref(fig:bagplot) shows the bagplot from the same 10 observations as were used in the illustration of depth in the previous section, along with the observations themselves shown in black.

(ref:bagplot) A bagplot of the 10 bivariate observations from Figure \@ref(fig:tukeydepth). The depth median is shown as the blue point in the centre. The darker shaded region is the bag, while the lighter shaded region shows the loop. There are no outliers outside the loop for this data set.

```{r bagplot, fig.cap="(ref:bagplot)"}
n01 %>%
  head(10) %>%
  gg_bagplot(v1, v2) +
...
A more interesting example is obtained in Figure \@ref(fig:oldfaithfulbagplot), showing a bagplot of the durations and waiting times of Old Faithful eruptions.
