...
The first approach to this problem was (again) due to John Tukey [@Tukey75] who defined the depth of a point $\bm{y}_k$ in a data set $\{\bm{y}_1,\dots,\bm{y}_n\}$ as the smallest number of $\bm{y}_i$ contained in any  halfspace that contains $\bm{y}_k$. This is often called the "Tukey depth" of the point. 

For bivariate data, a half space is either of the two parts formed by splitting the plane with a straight line.
A diagram will help illustrate the idea. Suppose we have the observations shown in Figure \@ref(fig:tukeydepth). These are the first 10 observations from the first two variables of `n01`. The red line divides the plane into two sections which are called "half spaces". 

```{r tukeydepth, echo=FALSE, fig.cap="A data set of 10 bivariate observations. The dividing red line splits the plane into two halfspaces."}
n01[1:10,] %>%
  ggplot(aes(x=v1,y=v2)) +
  geom_point() +
  geom_line(aes(x=x,y=y),data=data.frame(x=c(-1.7,0.3),y=c(-2,2)), col='red') +
  geom_point(data=n01[10,], col='orange', size=2) +
  geom_point(data=n01[5,], col='blue', size=2) +
...
Suppose we want to find the depth of the orange point. We would need to find the line which divides the plane into two sections where the half containing the orange point includes as few observations as possible. In fact, the red line is one such solution which has the orange point in a halfspace containing only 2 points. There are no dividing lines that would put the orange point in a halfspace on its own. So it has a Tukey depth of 2.

On the other hand, the blue point has depth 4 because there is a dividing line that will put it into a halfspace containing four observations but none that will contain fewer observations

In this data set, points have Tukey depths of 1, 2 or 4.

The Tukey depth median is the deepest point, in this case, the blue point of depth 4. 

### Convex hull peeling depths

Imagine that each of the observations shown in Figure \@ref(fig:tukeydepth) is a nail hammered into a board and we are looking at it from above. A rubber band wrapped around all observations will form a "convex hull" as shown on the left of Figure \@ref(fig:convexhull).
...
The depth region $D_k$ is the set of all points $\bm{z}$ with $\text{depth} \ge k$. For both of these depth measures, the depth regions are convex polygons and $D_{k+1} \subset D_k$. In the case of the convex hull peeling depths, the depth regions are equal to the convex layers. 

Figure \@(fig:tukeydepthregions) shows the depth regions for the two depth measures we have considered.
