...
Distance-based anomaly detection methods are based on the distance between observations. The pairwise distances between observations are computed, and anomalies are identified as those observations that are far from other observations.
...
Many anomaly detection algorithms are based on pairwise distances between observations. Suppose our observations are denoted by $\bm{y}_1,\dots,\bm{y}_n$. In univariate data, the distance between any two observations is simply the absolute difference between the points. But in multivariate data, there are many different ways of measuring distances between observations.
...
For example, here is the Minkowsi distance with $p = 3$ for the same five observations.
...
When the variables have different scales, the variables with the largest ranges will dominate the distance measures. In this example, durations are much longer than waiting times, and so the duration variable is dominating the calculation of neighbours. Consequently, it is often preferable to scale the data before computing distances.

We could just scale each variable separately, by subtracting the means and dividing by the standard deviations. However, this does not take into account the relationships between variables. To account for correlations as well as scale, we can use Mahalanobis distances, named after the Indian statistician Prasanta Chandra Mahalanobis. If our observations are from a probability distribution with covariance given by $\bm{\Sigma}$, then the Mahalanobis distance between $\bm{y}_i$ and $\bm{y}_j$ is given by
$$
  \|\bm{y}_i - \bm{y}_j\|_{\bm{\Sigma}} = \sqrt{(\bm{y}_i - \bm{y}_j)' \bm{\Sigma}^{-1} (\bm{y}_i - \bm{y}_j)}.
$$
This is a multivariate generalization of the number of standard deviations between any two observations. Let $\bm{\Sigma}^{-1} = \bm{U}' \bm{U}$ be the Cholesky decomposition of $\bm{\Sigma}^{-1}$. Then the data can be scaled by $\bm{U}$, giving $\bm{z}_i = \bm{U} \bm{y}_i$, so that the covariance matrix of the scaled data is the identity matrix. Then the Mahalanobis distances can be written as the Euclidian distances between the scaled observations
$$
  \|\bm{y}_i - \bm{y}_j\|_{\bm{\Sigma}} = \sqrt{(\bm{y}_i - \bm{y}_j)' \bm{U}' \bm{U} (\bm{y}_i - \bm{y}_j)} = \| \bm{z}_i - \bm{z}_j\|_2.
$$

Mahalanobis distance is not an option provided by the `dist()` function, so we will need to compute the scaled data first. Because there are anomalies in the data set, we will use the robust method given by @eq-ogk for estimating the covariance matrix.
...
library(robustbase)
of <- oldfaithful |> select(-time) 
S <- covOGK(of, sigmamu = s_IQR)
U <- chol(solve(S$cov))
z <- as.matrix(of) %*% t(U)
z |> head(5) |> dist()
```

Because we will often scale the data before computing distances, the weird package contains a `mvscale()` function to do this for us.
