...
Suppose we scale the data using the approach described in @sec-scaling, so that the scaled data are given by
$\bm{z}_i = \bm{U} \bm{y}_i$, where $\bm{U}'\bm{U} = \bm{\Sigma}^{-1}$ is the Cholesky decomposition of the inverse of the covariance matrix of the data. Then the Euclidean distance between $\bm{z}_i$ and $\bm{z}_j$ is given by
$$
  \| \bm{z}_i - \bm{z}_j\|_2 = \sqrt{(\bm{y}_i - \bm{y}_j)' \bm{U}' \bm{U} (\bm{y}_i - \bm{y}_j)} = \|\bm{y}_i - \bm{y}_j\|_{\bm{\Sigma}}
$$
This is known as the Mahalanobis distance, named after the Indian statistician Prasanta Chandra Mahalanobis.  It is a multivariate generalization of the number of standard deviations between any two observations.
