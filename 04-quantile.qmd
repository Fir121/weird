...
A naive approach that is sometimes used to find anomalies in univariate data is to identify the observations that are above or below a specified quantile value. For example, we might identify anomalies as observations above the 0.999 quantile or below the 0.001 quantile. More generally, an anomaly could be defined as observations above $\hat{Q}(1-p)$ or below $\hat{Q}(p)$.
