...
The R function `quantile()` includes nine variations, based on @HF96. Which variation you use makes little difference except for tiny data sets where $n$ is very small, or when you are interested in the tails when $p$ is close to 0 or 1. For anomaly detection, we are usually interested in the tails, so we will need to use a method that works well for extreme values of $p$.
...
Let $y_{(k)}$ denote the $k$th largest observation, $k=1,\dots,n$. These ordered values are known as "**order statistics**". Then the sample quantile $\hat{Q}(p)$ is given by linear interpolation between consecutive order statistics. That is, if $k^+= p(n+a) +b$, then
$$
  \hat{Q}(p) = (1-\gamma) y_{(k)} + \gamma y_{(k+1)},
$$
where $k = \lfloor k^+ \rfloor$ is the integer part of $k^+$ and $\gamma = k^+ - k$ is the remainder. Most of the different methods for estimating quantiles differ only in the values of $a$ and $b$.

* When the default `type=7` is used, $a = -1$ and $b=1$, so $k^+ = p(n-1) + 1$.
* When `type=8` is used, $a= b = 1/3$, so $k^+ = p(n+1/3) + 1/3$.

In either case, when $p=0.5$, we obtain the sample median:

  * If $n$ is even, then $k=n/2$ and $\gamma = 1/2$, so $\hat{Q}(0.5)$ is the average of the middle two observations.
  * If $n$ is odd, then $k=(n+1)/2$ and $\gamma = 0$, so $\hat{Q}(0.5)$ is the middle observation.

For large $n$, the sample quantile should provide a good estimate of the true quantile. That is, $\hat{Q}(p) \approx Q(p)$.  We can study the bias of the estimator by simulating from a N(0,1) distribution, and comparing the sample quantile to the true quantile. We repeat this 10,000 times, and average the results, in order to get a good estimate of the bias. The results are shown in @fig-quantile-bias.
...
qbias <- tibble(n = c(100, 1000))  |>
  purrr::pmap_dfr(~ avequantile(n = .x,
...
quartiles <- sprintf("%2.2f", quantile(batave$Average, c(seq(0, 1, by = 0.25), 0.99), type = 8))
n <- NROW(batave)
p <- c(seq(0, 1, by=0.25), 0.99)
kplus <- (n + 1/3) * p + 1/3
k <- trunc(kplus)
gamma <- kplus - k
```

There are $n=`r n`$ observations with a minimum of $y_{(1)} = `r quartiles[1]`$ and a maximum of $y_{(`r n`)} = `r quartiles[5]`$. Because there is an even number of observations, the sample median is the average of the middle two observations in the ordered data, which is
...
We can apply this idea to the cricket batting data using $p=1/1000$, again considering only those batters who have played at least 20 innings.
...
cricket_batting |>
  filter(Innings > 20) |>
  filter(
    Average < quantile(Average, prob = 0.001, type = 8) |
    Average > quantile(Average, prob = 0.999, type = 8)
  )  |>
...
This has identified the players with the lowest and highest averages. Remember, our earlier analysis suggested that only Don Bradman was really an anomaly. What has happened here, is that we were guaranteed to find one anomaly at each end of the distribution, because of the way sample quantiles are estimated. When $p$ is close to $1/n$, then $k=1$, and $\hat{Q}(p)$ is between $y_{(1)}$ and $y_{(2)}$. So there must be exactly one observation less than $\hat{Q}(p)$. Similarly, in the other tail of the distribution. In this example, $n= `r n`$, so $p=1/1000$ is close to $1/n$, leading to the two identified "anomalies".

In general, when $n$ is large, there will be $A = 2np$ observations less than $\hat{Q}(p)$ or greater than $\hat{Q}(1-p)$, regardless of how many anomalies there really are. The exact relationship between $A$ and $n$ is shown in @fig-spurious, for different values of $p$.

```{r}
#| label: fig-spurious
#| warning: false
#| fig.cap: Number of anomalies identified when looking for observations below $\hat{Q}(p)$ or above $\hat{Q}(1-p)$. The solid line shows the number of anomalies found using the `quantile()` function with `type=8`. The dashed line shows the line $A=2np$.
expand_grid(
    invp = 10^(2:6),
  ...
  scale_y_log10(
    breaks = c(0,2,4,10^(0:5)),
    labels = format(c(0,2,4,10^(0:5)), scientific = FALSE, trim = TRUE),
    limits = c(.1, 10^5.1),
    minor_breaks = NULL
  ) +
  coord_cartesian(ylim = c(0.34, 10^5.1), xlim=c(18,2.7e6)) +
  labs(
    x = "Sample size (n)",
...
1. There will always be about $2np$ observations identified as anomalies, regardless of how many anomalies there really are. The other sample quantile variations give worse results.
2. It doesn't allow us to find points that are inliers but otherwise unusual.
...
where $f(y)$ is the density of the underlying distribution [@van2000asymptotic, p308]. This is an asymptotic result, that assumes $np$ is large, and holds for both `type=7` and `type=8` sample quantiles.
...
The other sample quantiles as defined in @sec-sample-quantiles do not correspond exactly to specific depths, but @Tukey1977-xd introduced variations for some quantiles that are based on depths. He called these **letter values**. @hoaglin1983letter showed that  $\hat{Q}(p)$ gives approximately the same result as the corresponding letter value when using `type=8`.
...
Using the batting averages from @sec-sample-quantiles as an example, the depths of the first four letter values are given by
$$
  d_1 = `r d[1]`,\quad
  d_2 = `r d[2]`,\quad
...
\end{align*}
Note that the fourths are almost identical to the sample quartiles calculated earlier.
...
Here the median is given by M, the fourths by F, and so on. The middle box (F) is bounded by the fourths and contains all but 2/4 the data; the next box (E) is bounded by the eighths and contains all but 2/8 of the data; then D is bounded by the sixteenths and contains all but 2/16 of the data; and so on.
