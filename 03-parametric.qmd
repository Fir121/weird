...
3. The first 18 rows of the second variable in the `n01` data, along with the values 4.0 and 4.5. The latter two are anomalies as they are unlikely to arise from the N(0,1) distribution. The plot below shows the data for this example.
...
stopifnot(nanomalies$Zscore_robust == c(1, 243, 0, 1, 71, 24))
...
stopifnot(nanomalies$Peirce == c(0, 1, 2, 1, 8, 6))
```
...
stopifnot(nanomalies$Chauvenet == c(0, 1, 2, 1, 8, 7))
```
...
stopifnot(nanomalies$Grubbs == c(0, 1, 0, 0, 8, 1))
```

The clear failure is with the $t_2$ distribution which has no real anomalies. A spurious anomaly is also detected in the $\chi^2_4$ example, and real anomalies are missed in the Old Faithful data and in Example 3.
...
None of the tests found the two clear anomalies in the Old Faithful data, and most failed to find either of the artificial anomalies in Example 3. Only the Dixon test worked well for the large simulated data with no anomalies.
