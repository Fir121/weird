# Parametric methods {#ch-parametric-methods}
...
```{r zspurious, echo=FALSE, dependson="pspurious"}
#| label: fig-zspurious
...
```{r zspurious2, echo=FALSE, dependson=c("pspurious","zspurious")}
...
```{r zscoretests, echo=FALSE, dependson="pspurious"}
...
From these, we can compute the probability of a false positive (detecting an anomaly which is a genuine observation) in a normal distribution, using @eq-spuriousz. This probability should be equal to $\alpha=0.05$.

```{r pzscoretests, echo=FALSE, dependson=c("pspurious","zscoretests")}
#| label: fig-pzscoretests
#| fig-cap: True size of the tests (i.e., the probability of a false positive) for normally distributed data with $\alpha=0.05$.
...
```{r pgrubbs, echo=FALSE, dependson=c("pspurious","zscoretests")}
#| label: fig-pgrubbs
