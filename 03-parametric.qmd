...
5. a $\text{t}_2$ distribution, which has heavy tails;
...
```
 ...
 5. Twelve spurious anomalies have been identified in the 1000 $\text{t}_2$ observations.
...
If we replace the mean and standard deviation by more robust estimates (the median and IQR/1.349),^[The IQR is the Interquartile Range --- the difference between the 75th percentile and the 25th percentile of the data. See @sec-quantiles.] we obtain the same results in two of the examples, we find one of the two anomalies in `n01b`, but the remaining examples are even worse.
...
nanomalies <- nanomalies  |>
  mutate(Zscore_robust = c(
    cricket_batting |>
      filter(Innings > 20) |>
      mutate(z = (Average - median(Average)) / IQR(Average) * 1.349) |>
      filter(abs(z) > 3) |>
      NROW(),
    oldfaithful |>
      mutate(z = (duration - median(duration)) / IQR(duration) * 1.349) |>
      filter(abs(z) > 3) |>
      NROW(),
    n01b |>
      mutate(z = (y - median(y)) / IQR(y) * 1.349) |>
      filter(abs(z) > 3) |>
      NROW(),
    n01 |>
      mutate(z = (v1 - median(v1)) / IQR(v1) * 1.349) |>
      filter(abs(z) > 3) |>
      NROW(),
    t2 |>
      mutate(z = (y - median(y)) / IQR(y) * 1.349) |>
...
stopifnot(nanomalies$Zscore_robust == c(1, 243, 0, 2, 71, 24))
...
For the $\text{t}_2$ example, there are now `r nanomalies$Zscore_robust[5]` out of `r NROW(t2)` observations falsely identified as anomalous, while for the $\chi^2_4$ example, there are now `r nanomalies$Zscore_robust[6]` out of `r NROW(chisq4)` observations falsely identified as anomalous.
...
Let's also compute the probability of spurious anomalies for different data distributions --- the $\text{t}_2$ and $\chi^2_4$ distributions shown in @fig-testdist.
...
5. Eight spurious anomalies have been identified in the 1000 $\text{t}_2$ observations.
...
The clear failure is with the $\text{t}_2$ distribution which has no real anomalies. A spurious anomaly is also detected in the $\chi^2_4$ example, and real anomalies are missed in the Old Faithful data and in `n01b`.
...
For these examples, the test has worked relatively well apart from `n01b`, where it is failed to identify either anomaly.
...
library(kableExtra)
nanomalies$Example[5] <- "<span class='math inline'>\\text{t}_2</span>"
...
None of the tests found the two clear anomalies in the Old Faithful data, and most failed to find either of the artificial anomalies in `n01b`. Only the Dixon test worked well for the large simulated data with no anomalies.
