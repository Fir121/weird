# Parametric methods {#sec-parametric-methods}
...
3. The first 18 rows of the second variable in the `n01` data, along with the values 4 and 5. The latter two are anomalies as they are unlikely to arise from the N(0,1) distribution. The plot below shows the data for this example.

```{r}
#| label: fig-n01b
#| fig.asp: 0.2
#| fig.cap: "Nineteen observations from a N(0,1) distribution along with one anomaly."
n01b <- n01 |>
  select(v2) |>
  head(20)
n01b$v2[19:20] <- c(4,5)
...
4. a N(0,1) distribution (we will use the `n01` data set);
5. a $t_2$ distribution, which has heavy tails;
...
```
 ...
 5. Twelve spurious anomalies have been identified in the 1000 $t_2$ observations.
...
stopifnot(nanomalies$Zscore == c(1, 1, 0, 1, 12, 15))
```

If we replace the mean and standard deviation by more robust estimates (the median and IQR/1.35),^[The IQR is the Interquartile Range --- the difference between the 75th percentile and the 25th percentile of the data. See @sec-quantiles.] we obtain the same results in two of the examples, we find one of the two anomalies in Example 3, but the remaining examples are even worse.
...
stopifnot(nanomalies$Zscore_robust == c(1, 243, 1, 1, 71, 24))
...
For the Old Faithful example, there are now `r nanomalies$Zscore_robust[2]` out of `r NROW(oldfaithful)` observations identified as anomalous, including all of those below `r of_threshold` seconds.

For the $t_2$ example, there are now `r nanomalies$Zscore_robust[5]` out of `r NROW(t2)` observations falsely identified as anomalous, while for the $\chi^2_4$ example, there are now `r nanomalies$Zscore_robust[6]` out of `r NROW(chisq4)` observations falsely identified as anomalous.
...
In fact, we can be more accurate than this. If our $n$ observations come from a normal distribution, then the z-scores follow (approximately) a $t_{n-1}$ distribution. (The approximation arises because we replace the mean and standard deviations by their sample estimate.) When we identify anomalies as those points with $|z|>c$, then the probability of finding at least one spurious anomaly is the probability of the maximum being above $c$ or the minimum being below $-c$. Since the distribution is symmetric, these probabilities are the same. Let $M_n$ be the maximum of $n$ z-scores computed from the data; then $P(M_n \le c) = (F_{t}(c; n-1))^n$ where $F_{t}(y; n-1)$ is the cumulative distribution function of a $t$ distribution with $n-1$ degrees of freedom. So the probability of at least one spurious anomaly is
$$
  1 - (F_{t}(c; n-1))^{2n}.
$$ {#eq-spuriousz}
...
Let's also compute the probability of spurious anomalies for different data distributions --- the $t_2$ and $\chi^2_4$ distributions shown in @fig-testdist.
...
zspurious2df |>
  ...
  labs(y = "Probability of at least one spurious anomaly", x = "Sample size (n)") +
  scale_color_discrete(
    breaks = c("t(2)", "Chi-squared(4)", "Normal"),
    labels = c(
      latex2exp::TeX("$t_{~2}$"),
      latex2exp::TeX("$\\chi^{~2}_{~~4}$"),
      "N(0,1)"
...
Benjamin Peirce was a Harvard mathematician in the mid 1800s who worked with astronomical data which were prone to anomalous observations. In @Peirce1852 he proposed the first known test based on what are now called $z$-scores. His criteria was that any observations with $|z| > c$ should be "rejected", where $c$ is a complicated function depending on the sample size $n$ and the number of suspected anomalies.
...
nanomalies <- nanomalies  |>
  mutate(Peirce = c(
    ...
    n01b |> filter(peirce_anomalies(v2)) |> NROW(),
    n01 |> filter(peirce_anomalies(v1)) |> NROW(),
    t2 |> filter(peirce_anomalies(y)) |> NROW(),
    chisq4 |> filter(peirce_anomalies(y)) |> NROW()
  ))

stopifnot(nanomalies$Peirce == c(0, 1, 1, 1, 8, 6))
```
...
3. The most extreme anomaly has correctly been spotted amongst the 18 N(0,1) observations, but not the other anomaly.
4. Again, one spurious anomaly is identified in the 1000 N(0,1) observations.
5. Eight spurious anomalies have been identified in the 1000 $t_2$ observations.
...
The `chauvenet_anomalies()` function can be used to implement this test. For our six examples, it gives similar results to those above for `peirce_anomalies()` (with one additional spurious anomaly for the $\chi^2_4$ example.)
...
nanomalies <- nanomalies  |>
  mutate(Chauvenet = c(
    ...
    n01b |> filter(chauvenet_anomalies(v2)) |> NROW(),
    n01 |> filter(chauvenet_anomalies(v1)) |> NROW(),
    t2 |> filter(chauvenet_anomalies(y)) |> NROW(),
    chisq4 |> filter(chauvenet_anomalies(y)) |> NROW()
  ))

stopifnot(nanomalies$Chauvenet == c(0, 1, 1, 1, 8, 7))
```
...
nanomalies <- nanomalies  |>
  mutate(Grubbs = c(
    ...
    n01b |> filter(grubbs_anomalies(v2)) |> NROW(),
    n01 |> filter(grubbs_anomalies(v1)) |> NROW(),
    t2 |> filter(grubbs_anomalies(y)) |> NROW(),
    chisq4 |> filter(grubbs_anomalies(y)) |> NROW()
  ))

stopifnot(nanomalies$Grubbs == c(0, 1, 1, 0, 8, 1))
```

The clear failure is with the $t_2$ distribution which has no real anomalies. A spurious anomaly is also detected in the $\chi^2_4$ example, and real anomalies are missed in the Old Faithful data.
...
nanomalies <- nanomalies  |>
  mutate(Dixon = c(
    ...
    n01b |> filter(dixon_anomalies(v2)) |> NROW(),
    n01 |> filter(dixon_anomalies(v1)) |> NROW(),
    t2 |> filter(dixon_anomalies(y)) |> NROW(),
    chisq4 |> filter(dixon_anomalies(y)) |> NROW()
  ))

stopifnot(nanomalies$Dixon == c(1, 1, 0, 0, 0, 0))
```

For these examples, the test has worked relatively well apart from Example 3, where it is failed to identify either anomaly.
...
#| echo: false
#| tbl-cap: "Number of anomalies detected by each test. Cells in red indicate that the test has failed to identify all known anomalies and no spurious anomalies."
library(kableExtra)
nanomalies$Example[5] <- "Ex 5: $t_2$"
nanomalies$Example[6] <- "Ex 6: $\\chi^2_4$"
nanomalies |>
  knitr::kable(escape = FALSE, format = "html") %>%
  add_header_above(c(" " = 3, "Anomaly detection method" = 6)) %>%
  column_spec(4, color = if_else(nanomalies$Zscore != nanomalies$`Expected anomalies`, "red", "black")) %>%
  column_spec(5, color = if_else(nanomalies$Zscore_robust != nanomalies$`Expected anomalies`, "red", "black")) %>%
  column_spec(6, color = if_else(nanomalies$Peirce != nanomalies$`Expected anomalies`, "red", "black")) %>%
  column_spec(7, color = if_else(nanomalies$Chauvenet != nanomalies$`Expected anomalies`, "red", "black")) %>%
  column_spec(8, color = if_else(nanomalies$Grubbs != nanomalies$`Expected anomalies`, "red", "black")) %>%
  column_spec(9, color = if_else(nanomalies$Dixon != nanomalies$`Expected anomalies`, "red", "black"))
```

None of the tests found the two clear anomalies in the Old Faithful data, and none found both artificial anomalies in Example 3. Only the Dixon test worked well for the large simulated data with no anomalies.
