...
3. The first 19 rows of the second variable in the `n01` data, along with the value 4. The latter is an anomaly as it is unlikely to arise from the N(0,1) distribution. The plot below shows the data for this example.
...
5. a t(2) distribution, which has heavy tails;
...
Good anomaly detection methods should pick up one anomaly in the cricket batting example, at least two anomalies in the Old Faithful example, and the anomaly in the `n01` example. They should identify no anomalies in the remaining examples.
...
Let's see what happens when we apply this test to the six examples, with 1000 observations drawn from the three distributions.
...
```
 ...
 3. The anomaly added to the 19 N(0,1) observations has been correctly identified.
 4. One spurious anomaly has been identified in the 1000 N(0,1) observations.
 5. Twelve spurious anomalies have been identified in the 1000 t(2) observations.
...
For the Old Faithful example, there are now `r n2` out of `r NROW(oldfaithful)` observations identified as anomalous, including all of those below `r of_threshold` seconds.

For the $t(2)$ example, there are now `r n5` out of `r NROW(t2)` observations falsely identified as anomalous, while for the $\chi^2_4$ example, there are now `r n6` out of `r NROW(chisq4)` observations falsely identified as anomalous.
