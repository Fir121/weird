...
Let's see what happens when we apply this test to the two simple examples introduced in Chapter \@ref(ch:intro).
...
In the first example, Don Bradman is correctly identified as the only outlier in the data, but in the second example only the extreme 2-hour duration is identified. We already know there was an additional strange 1-second eruption, plus several unusual eruptions between 140 and 180 seconds in length. If we replace the mean and standard deviation by more robust estimates (the median and IQR/1.34), we obtain even worse results:

```{r zscore2}
cricket_batting %>%
  filter(Innings > 20) %>%
  mutate(z = (Average - median(Average))/
                (IQR(Average)/1.34)) %>%
  filter(abs(z) > 3) %>%
  select(Player, Average, z)
...
The `peirce_anomalies()` function returns a logical vector indicating which observations are anomalous under this criterion. For example, when applied to the Old Faithful eruption durations, it finds only the most extreme duration.

```{r peirce_oldfaithful}
oldfaithful %>% 
...
The `chauvenet_anomalies()` function can be used to implement this test. In the two examples above, it identifies exactly the same anomalies as the `peirce_anomalies()` function. 
