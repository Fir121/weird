...
of <- oldfaithful |>
  filter(duration < 7000)
den <- density(of$duration)
hdr <- hdrcde::hdr(prob = c(50,99), den = den)
den_df <- tibble(x = den$x, y = den$y)
of |>
  ggplot(aes(x = duration)) +
  geom_density() +
  geom_line(data = tibble(x = hdr$hdr[1,1:2]), aes(x = x, y = 0),
            col = "#0072B2", linewidth = 2, alpha = 0.5) +
  geom_line(data = tibble(x = hdr$hdr[1,3:4]), aes(x = x, y = 0),
            col = "#0072B2", linewidth = 2, alpha=0.5) +
  geom_line(data = tibble(x = hdr$hdr[2,1:2]), aes(x = x, y = 0.00025),
...
hdr_df <- hdr_df |>
  pivot_longer(-prob, names_to = "names", values_to = "ends") |> 
  mutate(
  ...
  select(-names) |> 
  filter(!is.na(ends)) |> 
  bind_rows(tibble(prob = c(0,0), type = c("lower", "upper"),
                   ends = rep(hdr$mode,2))) |> 
  arrange(prob, ends) |>   
  mutate(n = 1+trunc((row_number()/2.01))) |> 
  pivot_wider(id_cols = c("n","prob"), names_from = type, values_from = ends) 
  

oldfaithful |>
  filter(duration < 7000) |>
  ggplot(aes(x = duration, y = 0)) +
  geom_jitter(width=0) +
  geom_ribbon(
    data = hdr_df,
    mapping = aes(x = )
...
for(i in seq_len(NROW(hdr_df)/2)) {
  p <- p + geom_ribbon(aes(
    x = ends[c(1,)
  ))
  
  
  (aes(x = hdr_df$ends[2*(i-1)+1],
                            xend = hdr_df$ends[2*i], 
                            y = -0.2, yend = 0.2, col=hdr_df$prob[i]))
}
p   +
  labs(y = "", x = "Duration (seconds)") +
  ylim(-5,5)
