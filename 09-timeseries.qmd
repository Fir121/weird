...
The Hampel filter is due to @Hampel1974, and is designed to replace anomalies in a time series with a estimate based on the surrounding observations. It identifies anomalies by computing the difference between each observation and a moving median. These differences are then standardized by dividing by a moving median absolute deviation (MAD) of the differences. Observations with standardized residuals greater than a specified threshold are considered outliers. The Hampel filter then replaces the outliers with the moving median.

The `hampel_anomalies()` function does the first part of this process, by identifying anomalies within a time series using moving medians and MADs.

```{r}
fr_mortality <- fr_mortality |>
  group_by(Age, Sex) |>
  mutate(
    hampel = hampel_anomalies(Mortality, bandwidth = 7, k = 5)
  )
```
