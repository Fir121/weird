...
For example, we could choose the bandwidth matrix to be 
$$
	\bm{H} = 2h_* \bm{I}.
$$
Then
\begin{align*}
K_H(\bm{y} - \bm{y}_i) 
  &= (2\pi)^{d/2} \det(\bm{H})^{-1/2} \exp\left(-\frac{1}{2}(\bm{y} - \bm{y}_i)^T \bm{H}^{-1} (\bm{y} - \bm{y}_i)\right) \\
  &= \left(\frac{\pi}{h_*}\right)^{d/2} K\left(\frac{\bm{y} - \bm{y}_i}{2h_*}\right) 
\end{align*}
so that points more than $h_*$ away have little influence on the density estimate.
...
annulus_density <- X |>
  kde(H = 2*top2[1] * diag(2))
annulus_density |>
  autoplot() +
  geom_point(aes(x, y), data = X, alpha=0.15)
```

In this example, we have simply used a diagonal bandwidth matrix with the same bandwidth in each direction. That works for the annulus example, because the variables are on the same scale, and there is no correlation between them. However, in general we will want to use a bandwidth matrix that is not diagonal, and that allows for correlation between the variables.
...
$\Delta_i = d_{(i+1)} - d_{(i)}$, for $i=1,\dots,n-1$. We choose $h_*$ to be the diameter corresponding to the largest $\Delta_i$, and let $\bm{V}$ denote the diagonal matrix with entries equal to the diagonals of $\bm{S}_{\text{OGK}}$. Then we can define the bandwidth matrix to be
$$
  \bm{H} = \left(\frac{4}{d+2}\right)^{2/(d+4)} n^{-2/(d+4)} h_*\bm{V}^{-1/2}\bm{S}_{\text{OGK}}.
$$ {#eq-lookoutH}
Note that this is of the same form as the normal reference rule (@eq-gaussianH), but with a robust covariance matrix estimate that has been scaled to account for topological data structures.


We illustrate using the Old Faithful data discussed earlier.
