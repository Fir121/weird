...
grid <- seq(-5.5, 5.5, l = 199)
den <- list(x = expand_grid(y1 = grid, y2 = grid))
den$y <- mvtnorm::dmvnorm(den$x, mu, Sigma) 
# Find the HDR contours with probability 0.1, 0.2, ..., 0.9
prob <- seq(9)/10
hdr <- hdr_table(density = den, prob = prob)
# Plot the contours
den$x |> 
  mutate(z = den$y) |> 
  ggplot(aes(x = y1, y = y2, z = z)) +
...
Sigma2 <- diag(2)
# Define the density function on a grid
den <- list(x = expand_grid(y1 = seq(-3, 6, l = 199), y2 = seq(-3, 4, l = 199)))
den$y <- p1 * dmvnorm(den$x, mu1, Sigma1) + p2 * dmvnorm(den$x, mu2, Sigma2)
# Find the HDR contours with probability 0.1, 0.2, ..., 0.9
prob <- seq(9) / 10
hdr <- hdr_table(density = den, prob = prob)
# Plot the contours
...
H <- diag(c(MASS::bandwidth.nrd(of2021$duration),
       MASS::bandwidth.nrd(of2021$waiting))^2/16)
```
...
of2021 |>
  ...
  geom_point() +
  geom_density_2d(color = "#0072B2", h = 4*h) + 
  labs(x = "Duration (seconds)", y = "Waiting time (seconds)") +
  xlim(140, 290) +
  ylim(3010, 6860)
```
