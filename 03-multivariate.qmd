...
Because there may be anomalies in the data set, we will estimate $\bm{\mu}$ using the pointwise median (i.e., the vector of medians for each variable), and $\bm{\Sigma}$ using the robust covariance matrix $\bm{S}_{\text{OGK}}$ (@eq-ogk). The `mvscale()` function in the `weird` package will do this for us.
