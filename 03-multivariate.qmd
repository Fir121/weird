...
When first thinking about multivariate probability distributions, it can take some time to develop an intuition for how they behave. One way to start developing this intuition is to first consider univariate case, and then extend the ideas to higher dimensions.
...
First, we can do a strip plot of each variable to see their univariate marginal distributions.
...
Now we will plot pairwise scatterplots of the four variables.
...
To visualise three-way relationships, we need to use a 3d-scatterplot. It is easiest to see this if it can spin. Try dragging the plot around with your mouse to view if from different angles. See if you can find an viewpoint where the second anomaly is clearly separated from the other observations.
...
#| webgl: true
#| code-fold: false
#| fig-cap: 3d scatterplot of the three variables, with anomalies shown in orange.
...
#| label: fig-curse3d
#| webgl: true
#| fig-cap: 2000 simulated observations from three independent U(-1,1) distributions, with points within the unit sphere shown in grey, and points outside the unit sphere shown in orange.
df <- tibble(
  x = runif(2000, -1, 1),
  y = runif(2000, -1, 1),
  z = runif(2000, -1, 1),
...
As the number of dimensions grows, it becomes increasingly likely that points will lie in the corners of the space. In fact, the proportion of points lying outside the unit sphere in $d$ dimensions is
$$
1 - \frac{\pi^{d/2}}{2^d\Gamma(d/2 + 1)}
$$
where $\Gamma$ is the gamma function. This is plotted in @fig-curse.
...
Remarkably, by 10 dimensions, almost all points lie outside the unit sphere, and live in the corners of the space. Almost no points are in the center of the space. This a consequence of the curse of dimensionality.

Another way to think about this is to consider hypercubes which contain 50% of the observations. These must have sides of length $2^{1-1/d}$, so that the volume of the hypercube is $2^{d-1}$, exactly half of the entire space.
...
The whole space has side length equal to 2, so the side lengths of these hypercubes increasingly approaches the maximum of 2 as the number of dimensions increases. 

Because of the curse of dimensionality, we cannot use methods that rely on local neighbourhoods in high dimensions, as these neighbourhoods must become so large as to no longer be "local".
