...
of_scores <- oldfaithful |>
  mutate(
    score = kde_scores(duration, 
      h = kde_bandwidth(duration, method = "lookout")),
    looscore = kde_scores(duration, 
      h = kde_bandwidth(duration, method = "lookout"), 
      loo = TRUE
    ),
  )
...
Low probabilities indicate likely outliers and high probabilities indicate normal points. This procedure has identified the minimum and maximum eruptions as clear outliers. All other observations have p-values greater than 0.05, and we can conclude that they are not anomalies. @fig-ofpot shows an HDR boxplot of the data, with those points identified as lookout anomalies highlighted in red.
...
The `lookout` package also implements this method, but uses the Epanechnikov kernel rather than the Gaussian kernel, and selects the bandwidth using a different approach.
