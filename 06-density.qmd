...
library(mgcv)
fit <- gam(prop_no ~ s(Innings), data = df_no,
           family = binomial, weights = Innings)
df_no <- df_no |>
  bind_cols(
    broom::augment(fit, type.predict = "response") |>
      select(-prop_no, -Innings)
  )
df_no |>
...
Now we can use the fitted model to compute the log scores, and find the most anomalous batters.
...
The `.fitted` value is the expected proportion of not outs for each player given the number of innings they have played, while `prop_no` gives the actual proportion of not outs they have had. The largest log score is for English batter Jimmy Anderson, who has had `r df_no$NotOuts[1]` not outs in `r df_no$Innings[1]` innings, which is much higher than the expected number of not outs of `r sprintf("%.2f", df_no$.fitted[1]*df_no$Innings[1])`. This anomaly is also seen in @fig-notouts-gam, as being somewhat unusual for that part of the data.

We have identified an anomaly that is not anomalous in the number of notouts, or in the number of innings, and the difference between the actual proportion and the predicted proportion is not anomalous either. Because we have used a statistical model, we have been able to account for the particular features of this data set, such as the discrete nature of the data, and the relationship between the variance and the mean as a function of the number of innings played.
