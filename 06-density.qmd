...
$$
where $𝟙(z)$ is an indicator function taking value 1 when $z$ is true and 0 otherwise. This probability is related to highest density regions discussed in @sec-hdr and @sec-mhdr. Suppose $R_\alpha$ is the highest density region whose boundary is the level set containing $y_i$, then $p_i = 1-\alpha$. If we want a false positive rate of $\alpha$, we can label $y_i$ as an anomaly if $p_i < \alpha$.
...
The function `score_tail_prob()` function will return values of $p_i$ given values of the density scores $g_i$, where the assumed density $f$ can be provided as a density function (such as `dnorm`) or an empirical estimate can be used.

For example, consider the 1000 values contained in the first column of `n01`. These were generated from a standard normal distribution.
