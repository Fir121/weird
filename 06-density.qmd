...
The "surprisal" [@Tribus1961] of an observation $\bm{y}_i$, is defined as
$$
  s_i = -\log f(\bm{y}_i),
$$ {#eq-surprisal}
...
We can use surprisals to compute the probability of an observation being at least as anomalous as $y_i$ under the distribution $f$:
$$
p_i = \int f(u) 𝟙(-\log f(u) > s_i) du = \int f(u) 𝟙(f(u) < f(y_i)) du,
$$ {#eq-pi-density}
where $𝟙(z)$ is an indicator function taking value 1 when $z$ is true and 0 otherwise.

```{r}
#| label: fig-density-scores
#| fig-cap: "Top: Density with one observation shown at $y_i$. The shaded area shows the area where $f(y) < f(y_i)$, corresponding to $p_i$. Middle: surprisals for different values of $y$. Bottom: surprisal probabilities for different values of $y$. In this example, $p_i = 0.1$."
#| fig-asp: 1.3
#| out-width: "80%"
#| echo: false
den <- dist_chisq(5)
...
The `surprisals()` function takes values of the data $y_i$ and returns values of either $s_i$ or $p_i$ (depending on the `probability` argument), where the assumed density $f$ can be provided as a distribution object from the `distributional` package.

### Connection to Highest Density Regions

Surprisal probabilities are related to highest density regions discussed in @sec-hdr and @sec-mhdr. If $y_i$ lies on the boundary of the $100(1-\alpha)\%$ highest density region, then $p_i = 1-\alpha$. Therefore, if we want a false positive rate of $\alpha$, we can label $y_i$ as an anomaly if $p_i < \alpha$.

### Symmetric distributions

For any symmetric distribution, $p_i = 2[1 - F(c+|y_i-c|)]$, where $F$ is the corresponding distribution function centered at $c$. Therefore, under the Normal distribution $N(\mu, \sigma^2)$, $p_i = 2[1-\Phi(|y_i-\mu|/\sigma)]$, where $\Phi$ is the standard Normal distribution function.

### Connection to z-scores

The above result shows that when $f$ is a normal density, identifying anomalies as observations with $p_i < \alpha$ is equivalent to a z-score test (@sec-zscores) with the threshold set to the $1-\alpha/2$ quantile of the standard Normal distribution.
...
If we do not know what the distribution of the data should be, or if we think our assumed distribution is incorrect for some reason, there are some ways to find approximate surprisal probabilities that are still accurate even when the assumed distribution $f$ is not.

Let $f^*$ denote the assumed (and possibly incorrect) distribution. It might be a poor estimate of the true distribution, or it might just be a distribution on the same sample space as the data. Using this assumed distribution, we can compute the surprisals, $g_i = -\log f^*(\bm{y}_i)$.

Then we fit a Generalized Pareto Distribution (GPD) to the largest surprisals using the POT approach discussed in @sec-evt. For example, we may fit a GPD using the $90^{\text{th}}$ percentile as the threshold. We apply the fitted GPD distribution to the surprisals to obtain the surprisal probability of each observation.

Even when $f^*$ is incorrect or poorly estimated, the extreme surprisals will still follow a Generalized Pareto Distribution. So it is a safe choice.

The approach does require a relatively large number of surprisals to be available. We are typically only using the top 10% of observations in estimating the GPD, so we would normally need at least a few hundred observations to start with.

The `surprisals()` function has an approximation argument for specifying if we want to use the GPD approximation in computing surprisal probabilities.

### Approximate empirical surprisal probabilities

An alternative approach that does not require a GPD is to estimate $p_i$ as the proportion of surprisals larger than $s_i$. For large $n$, this provides a quick and convenient estimate of $p_i$ that is accurate even when the assumed distribution $f^*$ is not. Suppose $f$ is the true (unknown) density, and $f^*$ is the assumed (possibly incorrect) density. Then for the $p_i$ values to be accurate, we only need that the ordering of $-\log f(y_i)$ and $-\log f^*(y_i)$ to be similar in the tails of the distribution.
...
We will use all three methods described above using the correct density in computing the surprisals, and then repeat the calculation using an incorrect density. For the incorrect density, we will use a $t_3$ distribution, which has much longer tails than the true $N(0,1)$ distribution.
...
from which we obtain the log-LOO regression surprisals given by
$$
  p_{-i} = -\log \phi(r_{-i})
...
broom::augment(fit) |>
  mutate(
    ...
    loo_surprisals = -log(dnorm(studentized_residuals, log = TRUE))
  )
```
...
wine_aug <- wine_aug |>
  mutate(
    ...
    LOO_surprisals = -dnorm(Studentized_residuals, log = TRUE)
  )
wine_aug |>
  select(points, Studentized_residuals, LOO_surprisals, location) |>
  tidyr::pivot_longer(c(Studentized_residuals, LOO_surprisals), names_to = "variable", values_to = "value") |>
  mutate(variable = factor(variable, levels = c("Studentized_residuals", "LOO_surprisals"))) |>
...
wine_aug |>
  ...
  select(country:winery, year, points, price, Studentized_residuals, LOO_surprisals)
```
...
Suppose, instead of a regression or a GAM, we estimate $f$ using a kernel density estimate. In fact, when the `surprisals()` functions is applied to data without specifying a probability distribution, a kernel density estimate of the data is used. The kernel density estimate at each observation is (@eq-mkde)
$$
...
of_surprisals <- oldfaithful |>
  mutate(
    kde_surprisal = surprisals(duration, loo = FALSE, probability = FALSE),
    loo_kde_surprisal = surprisals(duration, loo = TRUE, probability = FALSE),
    prob = surprisals(duration, loo = TRUE),
  )
of_surprisals |> arrange(desc(loo_score))
```

The two infinite LOO surprisals correspond to the extreme 2 hour duration, and the tiny 1 second duration. These are so improbable given the rest of the data, that the scores are effectively infinite. The regular kde scores (with `loo = FALSE`) are at their maximum values. The `prob` column contains the surprisal probabilities using the `loo_kde_surprisal` values.
...
```{r}
#| label: fig-ofpot2
#| fig.cap: "HDR scatterplot of the Old Faithful eruption durations and waiting times, with the surprisal anomalies highlighted in red."
of |>
