...
The **t** distribution was first described by the English statistician and chemist, William Gosset, who worked for the Guinness brewery in Dublin, Ireland, and published under the pseudonym "Student". It arises naturally as the ratio $Z/\sqrt{S/k}$, where $Z\sim N(0,1)$ and $S\sim \chi^2_k$ are independent, and is commonly used in variations of $t$-tests. It takes one parameter, the degrees of freedom $k$. The pdf is given by
$$
  f_k(y) = \frac{\Gamma\left(\frac{k+1}{2}\right)}{\sqrt{k\pi}\Gamma\left(\frac{k}{2}\right)}
  \left(1 + \frac{y^2}{k}\right)^{-\frac{k+1}{2}}.
$$
We refer to a t random variable with $k$ degrees of freedom as $Y \sim t_k$. The t distribution is similar in shape to the standard Normal distribution, but has heavier tails. When $k=1$, the t distribution is known as the "Cauchy" distribution (after the French mathematician Augustin-Louis Cauchy), which has undefined mean and variance. For $k=2$, it has mean 0 and infinite variance. For $k > 2$, it has mean $0$ and variance $k/(k-2)$. When $k\rightarrow\infty$, the t distribution converges to the standard Normal distribution.
...
The quantile functions for a standard normal distribution, a $t_5$ distribution, and a $\chi^2_5$ distribution are shown in @fig-quantile.

```{r}
#| label: fig-quantile
#| fig-cap: Quantile function for a standard Normal distribution, a $t_5$ distribution, and a $\chi^2_5$ distribution.
tibble(
    p = seq(0, 1, l = 501),
    qy1 = qnorm(p, 0, 1),
    qy2 = qt(p, 5),
