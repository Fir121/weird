...
falpha <- hdr.2d(x = samp[,1], y = samp[,2], den = den, prob = prob)$falpha
expand_grid(
    x = seq(-3,3,l=199),
    y = seq(-3,3,l=199)
  ) |>
  ...
  labs(x = latex2exp::TeX("$y_1$"), y = latex2exp::TeX("$y_2$"),
...
Similarly, we can obtain HDRs for a mixture distribution. Suppose we had two bivariate Normal distributions with means $(0,0)$ and $(2,2)$, and covariance matrices equal to  $\begin{bmatrix} 1 & 0.5 \\ 0.5 & 1 \end{bmatrix}$ and the identity matrix $\bm{I}_2$ respectively. Then the HDRs for an equal mixture of these two distributions is shown in @fig-bivariate-mixture.

```{r}
#| label: fig-bivariate-mixture
#| echo: false
#| message: false
#| fig-cap: Bivariate mixture distribution of two equally weighted Normal components with means $(0,0)$ and $(2,2)$, and covariance matrices $\Sigma = \begin{bmatrix} 1 & 0.5 \\ 0.5 & 1 \end{bmatrix}$ and $\bm{I}_2$ respectively. The HDR contours cover 10%, 20%, $\dots$, 90% of the probability mass.
library(mvtnorm)
library(hdrcde)
# Find the contours corresponding to HDRs with probability 0.1, 0.2, ..., 0.9
p1 <- 0.5
...
den <- list(x=seq(-5,7,l=999), y=seq(-5,7,l=999))
den$z <- matrix(
