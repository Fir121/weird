...
This chapter covers various statistical and computational tools that will be used in subsequent chapters, collected here for easy reference. Feel free to skip the chapter entirely, or just read those parts that are unfamiliar, and refer back to it as needed.

Our aim here is to provide a relatively informal introduction to some of the tools that will be used in the book. Some of this material (especially @sec-distributions and @sec-clt) should be familiar to all readers, but is covered here as a refresher, and to introduce some notation that will be used in later chapters. References will be provided for those who want to learn more about the topics covered here.
...
A continuous random variable can take values on an uncountably infinite space. For example, when $Y$ can take any real value, or when $Y$ can take any positive value. For a continuous random variable, it is easiest to start with the cdf, given by @eq-cdf. For continuous random variables, $F(y)$ is a continuous function of $y$, and is monotonically increasing in $y$. The probability density function (pdf) is defined as the derivative of the cdf, so $f(y) = F'(y)$, assuming the derivative exists at $y$. The pdf is non-negative, and integrates to 1.

If $Y$ takes only real values, then the probability that $Y$ lies in the interval $[a,b]$ is given by $\text{Pr}(a \leq Y \leq b) = \int_a^b f(y)dy$.
...
$$
  f(y) = \frac{1}{\sigma\sqrt{2\pi}} \exp\left(-\frac{(y-\sigma)^2}{2\sigma^2}\right).
$$
...
tibble(
  ...
  pivot_longer(-y, names_to = "norm", values_to = "density") |>
  ggplot(aes(x = y, y = density, col = norm)) +
  geom_line() +
  labs(x = "y", y = "Probability Density Function: f(y)") +
  guides(col = guide_legend(title = latex2exp::TeX("N($\\mu$,$\\sigma^2$)")))
```
...
$$
  f(y) = \frac{y^{k/2-1}e^{-y/2}}{2^{k/2}\Gamma(k/2)}.
$$
where $\Gamma(u) = \int_0^\infty x^{u-1}e^{-x}dx$ is the gamma function. For positive integers $u$, $\Gamma(u) = (u-1)!$. The $\chi^2$ distribution arises naturally as the sum of the squares of $k$ independent standard Normal random variables. We refer to a $\chi^2$ random variable with $k$ degrees of freedom as $Y \sim \chi^2(k)$.
...
#| label: fig-chisq
#| echo: false
#| fig-cap: Probability density functions for three $\chi^2$ distributions. As the degrees of freedom $k$ increases, the distribution becomes more symmetric, and converges to a Normal distribution.
tibble(
    y = seq(0, 15, l = 501),
    `x1` = dchisq(y, 1),
    `x2` = dchisq(y, 2),
  ...
  pivot_longer(-y, names_to = "chisq", values_to = "density") |>
  ggplot(aes(x = y, y = density, col = chisq)) +
  geom_line() +
  labs(x = "y", y = "Probability Density Function: f(y)") +
  guides(col = guide_legend(title = latex2exp::TeX("$\\chi^2$$(k)$"))) +
  coord_cartesian(ylim=c(0,1)) +
  scale_color_manual(
    values = discrete_colors[1:3],
    labels = c(latex2exp::TeX("$\\chi^2$$(1)$"),
...
The **t** distribution was first described by the English statistician and chemist, William Gosset, who worked for the Guinness brewery in Dublin, Ireland, and published under the pseudonym "Student". It arises naturally as the ratio $Z/S$, where $Z\sim N(0,1)$ and $S\sim\sqrt{\chi^2(k)/k}$ are independent, and is commonly used in variations of $t$-tests. It takes one parameter, the degrees of freedom $k$. The pdf is given by
$$
  f(y) = \frac{\Gamma\left(\frac{k+1}{2}\right)}{\sqrt{k\pi}\Gamma\left(\frac{k}{2}\right)}
  \left(1 + \frac{y^2}{k}\right)^{-\frac{k+1}{2}}.
$$
We refer to a t random variable with $k$ degrees of freedom as $Y \sim t(k)$. The t distribution is similar in shape to the standard Normal distribution, but has heavier tails. When $k=1$, the t distribution is known as the "Cauchy" distribution (after the French mathematician Augustin-Louis Cauchy), which has undefined mean and variance. When $k\rightarrow\infty$, the t distribution converges to the standard Normal distribution.
...
tibble(
  ...
  pivot_longer(-y, names_to = "t", values_to = "density") |>
  ggplot(aes(x = y, y = density, col = t)) +
  geom_line() +
  labs(x = "y", y = "Probability Density Function: f(y)") +
  guides(col = guide_legend(title = latex2exp::TeX("t($k$)")))
```
...
$$
  \Gamma(y) = \frac{\beta^\alpha}{\Gamma(\alpha)} y^{\alpha-1}e^{-\beta y}.
$$
We refer to a gamma random variable with shape $\alpha$ and rate $\beta$ as $Y \sim \text{Gamma}(\alpha, \beta)$. The mean and variance of a gamma random variable are $\alpha/\beta$ and $\alpha/\beta^2$ respectively.
...
tibble(
  ...
  pivot_longer(-y, names_to = "gamma", values_to = "density") |>
  ggplot(aes(x = y, y = density, col = gamma)) +
  geom_line() +
  labs(x = "y", y = "Probability Density Function: f(y)") +
  guides(col = guide_legend(title = latex2exp::TeX("Gamma($\\alpha$,$\\beta$)")))
```
...
If we can define the cumulative probability distribution, then we can define the quantiles of that distribution, given by
$$
  Q(p) = \inf\{y: F(y) \geq p\}.
$$
This definition is valid for both discrete and continuous random variables.
...
For a continuous random variable, the quantile function is equal the inverse of the cumulative distribution function, $Q(p) = F^{-1}(p)$. Thus $\text{Pr}(Y \le Q(p)) = p$.
...
For example, the pdf of the bivariate Normal distribution is given by
$$
f(x,y) = \frac{1}{2\pi \sqrt{1-\rho^2}} \exp\left\{-\frac{1}{2(1-\rho^2)}\left(x^2 - 2\rho x y + y^2\right)\right\},
$$
where $\rho$ is the correlation between $X$ and $Y$. We often write this as
$$
\begin{bmatrix}x\\y\end{bmatrix} \sim N(\bm{0}, \bm{\Sigma}),
$$
where $\bm{0} = \begin{bmatrix}0\\0\end{bmatrix}$ is the mean, and $\bm{\Sigma} = \begin{bmatrix}1 & \rho \\ \rho & 1\end{bmatrix}$ is the covariance matrix. Multivariate Normal distributions have the interesting property that the marginal distributions are also Normal.

Bivariate distributions can be visualized using contour plots. When plotting contours of a pdf, it is helpful to show contours corresponding to highest density regions (HDRs). HDRs are discussed in more detail in @sec-hdr. For now, it is sufficient to note that an HDR is a region of the sample space that contains a specified proportion of the probability mass, and where every point inside the region has higher density than any point outside the region.
...
library(mvtnorm)
library(hdrcde)
# Find the contours corresponding to HDRs with probability 0.1, 0.2, ..., 0.9
prob <- seq(9)/10
grid <- seq(-2.5, 2.5, l=999)
den <- list(x=grid, y=grid)
den$z <- dmvnorm(cbind(den$x,den$y), c(0,0), matrix(c(1,0.5,0.5,1),2,2)) |>
  matrix(nrow = length(grid), ncol = length(grid), byrow = TRUE)
falpha <- hdr.2d(x = grid, y = grid, den = den, prob = prob)$falpha
expand_grid(
    x = seq(-3.5,3.5,l=99),
    y = seq(-3.5,3.5,l=99)
  ) |>
  mutate(z = dmvnorm(cbind(x,y), c(0,0), matrix(c(1,0.5,0.5,1),2,2))) |>
  ggplot(aes(x = x, y = y, z = z)) +
  geom_contour_filled(breaks = rev(c(falpha, 100))) +
  labs(x = "x", y = "y", z = "f(x,y)") +
  scale_fill_manual(values = rev(viridisLite::viridis(10)),
...
A **highest density region** is defined as the region of the sample space where the density is higher than a given threshold [@HDR96], and are usually specified using the probability mass that they contain. For example, a 95% HDR is the region of the sample space that contains 95% of the probability mass, and where every point inside the region has higher density than any point outside the region.

Suppose we have a multivariate random variable $\bm{Y}$ with a smooth, continuous density function $f$. Then the $100(1-\alpha)$% HDR is the set
$$
  R_\alpha = \{\bm{y}: f(\bm{y}) \ge f_\alpha\}
$$
where $P(\bm{Y} \in R_\alpha) = 1-\alpha$.

HDRs are equivalent to level sets of the density function, and so are contours for bivariate density functions. An example of a bivariate density function with HDR contours is shown in @fig-bivariate.
