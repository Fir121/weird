...
A discrete random variable can take values on a finite, or countably infinite, space. That is, if we can list the values that the variable can take, it is a discrete random variable. For example, if $Y$ is a coin toss, it can take values in $\{$Heads, Tails$\}$. If $Y$ is a count variable, it can take values on the non-negative integers $\{0, 1, 2, \dots\}$.
