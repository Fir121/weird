...
We are interested in data obtained from a range of possible sample spaces, but let's start with the two simplest cases: univariate discrete and continuous random variables. A good introduction to probability distributions is provided by @Forbes2011, with more advanced treatments in @JKunidiscrete, @JKunicontinuous1 and @JKunicontinuous2.
...
$$
  Q(p) = \inf\{y: F(y) \geq p\}.
$$
This definition is valid for both discrete and continuous random variables. For a continuous random variable, the quantile function is the inverse of the cumulative distribution function, $Q(p) = F^{-1}(p)$. The quantile functions for a standard normal distribution, a $t(5)$ distribution, and a $\chi^2(5)$ distribution are shown in @fig-quantile.
...
Because the cdf of a discrete distribution is a step function, the inverse is also a step function. For example, the quantile function for a Poisson(5) variable is shown in @fig-quantile2.
...
The central limit theorem (CLT) is one of the most important results in statistics. It says that the sum of a large number of independent random variables, each with finite mean and variance, is approximately Normally distributed. This is a remarkable result as it does not require any assumptions about the underlying distribution of the random variables, other than that they have finite mean and variance. It is also a very useful result as it allows us to use the Normal distribution to approximate the distribution of many statistics.


The CLT is the basis of many statistical methods, including hypothesis testing, confidence intervals, and linear regression.
