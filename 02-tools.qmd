...
Let $Y$ denote a continuous random variable taking values in $\mathbb{R}$, with its cumulative probability distribution (cdf) given by
$$F(y) = \text{Pr}(Y <= y).
$$ {#eq-cdf}
...
tibble(
    ...
    pdf1 = dnorm(y, -2, 1),
    pdf2 = dnorm(y, 2, 1),
    fy = 1/3 * pdf1 + 2/3 * pdf2
  ) |> 
  ggplot(aes(x=y, y=fy)) +
  geom_line() +
  geom_line(aes(y=1/3*pdf1), linetype="dashed") +
  geom_line(aes(y=2/3*pdf2), linetype="dashed")  +
  labs(x = "y", y = "Probability Density Function: f(y)") 
```
...
interval2 <- hdrcde::hdr(prob = 90, 
  den=list(x=seq(0,20,l=501), y=dchisq(seq(0,20,l=501), 5))
  )$hdr |> 
  as.numeric()
falpha <- dchisq(interval2[1], 5)
...
tibble(
  ...
  geom_line() +
  labs(x = "y", y = "Probability Density Function: f(y)") +
  geom_line(data = tibble(x=interval1), aes(x = x, y=-0.002), col="blue", linewidth=2) +
  geom_line(data = tibble(x=interval2), aes(x = x, y=0.001), col="red", linewidth=2) +
  geom_line(data = tibble(x=rep(interval2[1], 2), y=c(0, falpha)),
            aes(x=x,y=y), col="red", linetype="dashed") +
  geom_line(data = tibble(x=rep(interval2[2], 2), y=c(0, falpha)),
            aes(x=x,y=y), col="red", linetype="dashed") +
  geom_hline(aes(yintercept=falpha), col="red", linetype="dashed") +
  scale_y_continuous(breaks = c(seq(0, 0.15, by=0.05), falpha),
                     minor_breaks = NULL,
                     labels = latex2exp::TeX(c(seq(0, 0.15, by=0.05), 
                                "$f_\\alpha$")),
                     ) +
...
p <- as_tibble(den) |> 
  ...
  geom_line() +
  labs(x = "y", y = "Probability Density Function: f(y)") +
  geom_line(data = tibble(x=interval1), aes(x = x, y=-0.0028), col="blue", linewidth=2) +
  geom_line(data = tibble(x=interval2[1:2]), aes(x = x, y=0.0018), col="red", linewidth=2) +
  geom_line(data = tibble(x=interval2[3:4]), aes(x = x, y=0.0018), col="red", linewidth=2) 
for(i in seq_along(interval2)) {
  p <- p + geom_line(data = tibble(x=rep(interval2[i], 2), y=c(0, falpha)),
            aes(x=x,y=y), col="red", linetype="dashed") 
}
p +
  geom_hline(aes(yintercept=falpha), col="red", linetype="dashed") +
  scale_y_continuous(breaks = c(seq(0, 0.2, by=0.1), falpha),
                     minor_breaks = NULL,
                     labels = latex2exp::TeX(c(seq(0, 0.2, by=0.1), 
                                "$f_\\alpha$")),
                     ) +
...
The joint distribution of $d$ random variables $Y_1,\dots,Y_n$ is defined by the joint cdf 
$$
F(y_1,\dots,y_d) = \text{Pr}(Y_1 \le y_1,\dots,Y_d \le y_d),
$$
while the joint density function is given by
$$
f(y_1,\dots,y_d) = \frac{\partial^n F(y_1,\dots,y_d)}{\partial y_1 \dots \partial y_d}.
$$

The marginal cdfs are defined by $F_i(y) = \text{Pr}(Y_i \le y)$, with corresponding marginal pdfs given by $f_i(y) = F'(y_i)$, $i=1,\dots,d$.

If the variables are independent, then the joint pdf is the product of the marginal pdfs, $f(y_1,\dots,y_d) = \prod_{i=1}^d f_i(y_i)$.

### Bivariate Normal distribution

For example, the pdf of the bivariate Normal distribution is given by
$$
f(y_1,y_2) = \frac{1}{2\pi \sqrt{1-\rho^2}} \exp\left\{-\frac{1}{2(1-\rho^2)}\left(y_1^2 - 2\rho y_1 y_2 + y_2^2\right)\right\},
$$
where $\rho$ is the correlation between $Y_1$ and $Y_2$. We often write this as
$$
\bm{y} = \begin{bmatrix}y_1\\y_2\end{bmatrix} \sim N(\bm{0}, \bm{\Sigma}),
$$
...
The conditional distribution of $Y$ given $X_1,\dots,X_n$ is defined by the conditional cdf
$$
 F(y|x_1,\dots,x_n) = \text{Pr}(Y \le y | X_1 = x_1,\dots,X_n = x_n).
$$
...
Many anomaly detection algorithms are based on pairwise distances between observations. If there are $n$ observations, then there are $n(n-1)/2$ pairwise distances to compute, so this is an $O(n^2)$ operation which can take a very long time for large $n$. There are approximate pairwise distance algorithms that are faster, but we will not discuss them here.
