...
The kernel density estimate can be used to identify regions of highest density in the data. We could simply take the estimated distribution and use the `hdr()` function to find the HDR regions with specified probability coverage. For example, we can compute the 90% HDR for the `oldfaithful` data set as follows:
...
of_density |>
  gg_density(show_hdr = TRUE, prob = 0.9, show_points = TRUE, jitter = TRUE, alpha = 0.2) +
  labs(x = "Duration (seconds)")
```
