...
$$  {#eq-ruleofthumb}
where $s$ is the sample standard deviation and $\text{IQR} = \hat{Q}(0.75) - \hat{Q}(0.25)$. This tends to work relatively well for most data sets. For the 10 observations in the example above, it gives $h = `r round(h, 1)`$, which is the value we used.
