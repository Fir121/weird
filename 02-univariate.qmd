...
The R function `quantile()` includes nine variations, based on @HF96. Which variation you use makes little difference except for tiny data sets where $n$ is very small, or when you are interested in the tails when $p$ is close to 0 or 1. However, for anomaly detection, we are usually interested in the tails, so we will need to use a method that works well for extreme values of $p$.
...
p <- as_tibble(den) |>
  ...
  geom_line(data = tibble(x = interval1), aes(x = x, y = -0.0028),
            col = "#0072B2", linewidth = 2) +
  geom_line(data = tibble(x = hdrinterval1), aes(x = x, y = 0.0018),
            col = "#D55E00", linewidth = 2) +
  geom_line(data = tibble(x = hdrinterval2), aes(x = x, y = 0.0018),
            col = "#D55E00", linewidth = 2)
...
When $h$ is too small, the density estimate is very rough with many peaks and troughs. When $h$ is too large, the density estimate is too smooth and we fail to see any features in the data. A popular choice for $h$ is Silverman's "rule-of-thumb" [@Silverman1986, p48] given by
$$
  h = 0.9 \min\left\{s, \frac{\text{IQR}}{1.34}\right\} n^{-1/5},
$$ {#eq-nrd0}
where $s$ is the sample standard deviation, and $\text{IQR} = \hat{Q}(0.75)-\hat{Q}(0.25)$ is the sample interquartile range. For the 10 observations in the example above, it gives $h = `r round(h, 1)`$, which is the value we used.
...
#| label: ofbw2
#| include: false
#| dependson: fig-oldfaithful4
h <- bw.nrd0(of$duration)
```
...
$$ {#eq-asymptotickde}
and so $h$ should decrease slowly as $n$ increases. Note that these conditions hold for @eq-nrd0.
...
However, this value of $h$ depends on the underlying density $f$ which we don't know. If $f$ is a Normal density with variance $\sigma^2$, then $R(f) = \frac{1}{2\sqrt{\pi}}\sigma^{-1}$ and $R(f'') = \frac{3}{8\sqrt{\pi}}\sigma^{-5}$. So if we use a Gaussian kernel, replace $\sigma$ by its sample estimate, and assume the underlying density has the same roughness as a Normal density, we obtain the "normal reference rule":
$$
  h = s\left(\frac{4}{3n}\right)^{1/5} = 1.06s n^{-1/5}.
$$ {#eq-nrr}
This can be computed using the `ks::hns()` function.

If we replace $s$ by a more robust estimate of the standard deviation given by $\min\{s, \text{IQR}/1.34\}$,
we obtain the bandwidth estimate computed by the `bw.nrd()` function, and available in `geom_density()` by setting `bw = "nrd"`.

Silverman's "rule-of-thumb" (@eq-nrd0) is similar, but also replaces 1.06 by 0.9, because Silverman argued that most densities are not as smooth as a Normal density, and so a smaller bandwidth is required. This variation is computed using the `bw.nrd0()` function, and is the default bandwidth used by `geom_density()`.

Another popular bandwidth choice is the Sheather-Jones "plug-in" bandwidth [@SJ91], obtained by replacing $R(f'')$ in @eq-opth by an estimate based on the data. For most data sets, this will give a smaller value than @eq-nrr. To use this bandwidth, set `bw = "SJ"` in `geom_density()`. It can also be computed using the `bw.SJ()` function. The bandwidth computed by `ks::hpi()` is similar, but uses a different estimate of $R(f'')$.
...
$$
This shows that larger bandwidths are required when $f(y)/[f''(y)]^2$ is relatively large, which often occurs in the extreme tails of a distribution. So bandwidths for anomaly detection tend to be larger than bandwidths for other purposes. Unless otherwise stated, we will use the normal reference rule (@eq-nrr) for examples in the rest of this book.

### Estimating the density using R

As we have seen, `geom_density()` allows us to plot the kernel density estimate, with the bandwidth specified using the `bw` argument and the kernel specified by `kernel`. If we want to save the estimate of the kernel density estimate for later analysis, we could use the `density()` function, where again the `bw` and `kernel` arguments control the bandwidth and the kernel.

However, we will use the `kde()` function from the `ks` package instead, as it also handles multivariate density estimation (discussed in @sec-multivariate). It uses a Gaussian kernel, and allows the bandwidth to be controlled via the `h` argument. The default bandwidth is given by `ks::hpi()`, implementing a variation of the Sheather-Jones plug-in rule.

For example, suppose we want to compute the kernel density estimate for the `oldfaithful` data set, using the normal reference rule (@eq-nrr), and then generate a graph similar to @fig-oldfaithful4 (but with a larger bandwidth). We can do this using the following code.

```{r}
#| label: fig-oldfaithful5
#| code-fold: false
#| fig-cap: Kernel density estimate for the duration of the Old Faithful eruptions, using the normal reference rule for the bandwidth.
library(ks)
of <- oldfaithful |>
  filter(duration < 7000)
of_density <- kde(of$duration, h = hns(of$duration))
tibble(
    x = of_density$eval.points,
    y = of_density$estimate
  ) |>
  ggplot() +
  geom_line(aes(x=x, y=y)) +
  geom_rug(data = of, aes(x=duration)) +
  labs(x = "Duration (seconds)", y = "density")
```

### Highest kernel density regions

The kernel density estimate can be used to identify regions of highest density in the data. We could simply take the estimated density function and use the `hdr_table()` function to find the HDR regions with specified probability coverage.

However, as noted above, the bandwidth used for the kernel density estimate is designed to give a good overall estimate of the underlying density, and we often need a larger bandwidth in the tails of the distribution.

@Samworth2010 proposed a method for finding the optimal bandwidth when estimating highest density regions of a distribution, which is implemented in the `hdrbw()` function in the `hdrcde` package. @fig-hdrbw shows the corresponding bandwidths obtained for the `oldfaithful` data for different values of the probability coverage, along with those obtained by the normal reference rule (@eq-nrr), the robust normal reference rule, Silverman's rule of thumb (@eq-nrd0), and the Sheather-Jones plug-in rule.

```{r}
#| label: fig-hdrbw
#| fig-cap: Optimal bandwidths (in black) for computing the highest density regions of the `oldfaithful` data set at different coverage probabilities, along with bandwidths specified by various methods.
bandwidths <- tibble(
  Method = c("Normal reference rule", "Robust normal reference rule",
           "Silverman's rule of thumb", "Sheather-Jones plug-in rule"),
  Bandwidth = c(hns(of$duration), bw.nrd(of$duration),
                bw.nrd0(of$duration), bw.SJ(of$duration))
  ) |>
  mutate(Method = fct_reorder(Method, Bandwidth, .desc = TRUE))
prob <- seq(0.5, 0.99, by=0.005)
bw <- numeric(length(prob))
for (i in seq_along(prob)) {
  bw[i] <- hdrcde::hdrbw(of$duration, prob[i])
}
tibble(Probability = prob, Bandwidth = bw) |>
  ggplot() +
  geom_line(aes(x=Probability, y=Bandwidth)) +
  geom_hline(data = bandwidths, aes(yintercept = Bandwidth, col = Method)) +
  labs(x = "HDR Probability Coverate", y = "Optimal bandwidth")
```

Clearly, most of the popular rules give bandwidths that are too small for the purpose of estimating the highest density regions. For this example, the normal reference rule gives a bandwidth suitable for an HDR of about 90% coverage. This was the value used when estimating the density shown in @fig-oldfaithful5, so we already have the density estimate we need.

We can compute the 90% HDR for the `oldfaithful` data set as follows:
...
The resulting estimate is shown in @fig-of-hdr, along with the jittered data.
...
of |>
  ggplot() +
  geom_jitter(aes(x = duration, y = 0),
              alpha = 0.5, width = 0, height = 0.8) +
  geom_rect(data = hdr,
    aes(xmin = lower, xmax = upper, ymin=-1, ymax=1),
    fill = "#00659e", alpha = 0.92
  ) +
  scale_y_discrete() +
...
When a Gaussian kernel is used, we are implicitly assuming that the underlying density $f$ is smooth and non-zero on the whole real line. This will cause problems when the true density is actually zero for some regions of the sample space. For example, if all data must be positive, then $f(u)=0$ for $u<0$.
