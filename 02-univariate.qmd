...
When $h$ is too small, the density estimate is very rough with many peaks and troughs. When $h$ is too large, the density estimate is too smooth and we fail to see any features in the data. A popular choice for $h$ is the "normal reference rule" [@Silverman1986, p45] given by
$$
  h = 1.06 s n^{-1/5}.
$$ {#eq-nrr}
where $s$ is the sample standard deviation. For the 10 observations in the example above, it gives $h = `r round(h, 1)`$, which is the value we used.
...
We will use the `kde()` function from the `ks` package, which uses the Gaussian kernel, and we will set the bandwidth using the normal reference rule (@eq-nrr) via the `hns()` function.
...
If we replace $s$ by a more robust estimate of the standard deviation, we obtain the bandwidth estimate
$$
  h = 1.06 \min\left(s, \frac{IQR}{1.34}\right) n^{-1/5}.
$$ {#eq-nrd}
...
@Samworth2010 proposed a method for finding the optimal bandwidth when estimating highest density regions of a distribution, which is implemented in the `hdrbw()` function in the `hdrcde` package. @fig-hdrbw shows the corresponding bandwidths obtained for the `oldfaithful` data for different values of the probability coverage, along with those obtained by the normal reference rule (@eq-nrr), the robust normal reference rule (@eq-nrd), Silverman's rule of thumb, and the Sheather-Jones plug-in rule.
...
for (i in seq_along(prob)) {
  bw[i] <- hdrcde::hdrbw(of$duration, 1-prob[i])
}
tibble(Probability = prob, Bandwidth = bw) |>
  ...
  labs(x = "HDR Probability Coverage", y = "Optimal bandwidth")
```

While it is increasingly difficult to estimate the required bandwidth as the probability coverage increases, it is clear that most of the popular rules give bandwidths that are too small for the purpose of estimating the highest density regions. For this example, the normal reference rule gives a bandwidth suitable for an HDR of between 75% and 90% coverage. This was the value used when estimating the density shown in @fig-oldfaithful4, so we already have the density estimate we need.
