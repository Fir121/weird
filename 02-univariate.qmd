...
of_density |>
  autoplot(show_hdr = TRUE, prob = 0.9, show_points = TRUE, jitter = TRUE, alpha = 0.2) +
  labs(x = "Duration (seconds)")
```
...
#| fig.cap: "Distribution of the maximum of 1000 N(0,1) draws. Here we have simulated 10000 such maximums and shown the estimated GEV distribution (in black), along with the true distribution (in blue). The approximation is so good, that it is hard to distinguish the two distributions."
maximums <- replicate(10000, max(rnorm(1000)))
gev <- evd::fgev(maximums)$estimate
dist_gev(location = gev["loc"], scale = gev["scale"], shape = gev["shape"]) |>
  gg_dist() +
  geom_line(
    data = tibble(y = seq(2, 7, l = 100), fy = n * pnorm(y)^(n - 1) * dnorm(y)),
    aes(x = y, y = fy),
    col = "#0072B2"
  )
```
