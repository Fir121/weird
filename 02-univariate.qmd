...
$$
This shows that larger bandwidths are required when $f(y)/[f''(y)]^2$ is relatively large, which often occurs in the extreme tails of a distribution. So bandwidths for anomaly detection tend to be larger than bandwidths for other purposes. Unless otherwise stated, we will use the robust normal reference rule (@eq-nrd) for examples in the rest of this book.
...
Clearly, most of the popular rules give bandwidths that are too small for the purpose of estimating the highest density regions. For this example, the normal reference rule gives a bandwidth suitable for an HDR of about 90% coverage. This was the value used when estimating the density shown in @fig-oldfaithful4, so we already have the density estimate we need.
