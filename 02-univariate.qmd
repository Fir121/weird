...
$$
This shows that larger bandwidths are required when $f(y)/[f''(y)]^2$ is relatively large, which often occurs in the extreme tails of a distribution. So bandwidths for anomaly detection tend to be larger than bandwidths for other purposes. For that reason, we will use the robust normal reference rule (@eq-rnrr), computed using the `kde_bandwidth()` function, in the rest of this book.
