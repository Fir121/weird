...
When $h$ is too small, the density estimate is very rough with many peaks and troughs. When $h$ is too large, the density estimate is too smooth and we fail to see any features in the data. A popular choice for $h$ (explained later) uses  
$$
  h = 1.06 s n^{-1/5},
$$  {#eq-nrr}
...
However, this value of $h$ depends on the underlying density $f$ which we don't know. For a Normal density $g$ with variance $\sigma^2$, $R(g) = \frac{1}{2\sqrt{\pi}}\sigma^{-1}$ and $R(g'') = \frac{3}{8\sqrt{\pi}}\sigma^{-5}$. So if we use a Gaussian kernel, replace $\sigma$ by its sample estimate, and assume the underlying density has the same roughness as a Normal density, we obtain the "normal reference rule" previously given by @eq-nrr:
$$
  h = s\left(\frac{4}{3n}\right)^{1/5} = 1.06s n^{-1/5}.
$$ {#eq-nrr2} 
To use this bandwidth in `geom_density()`, set `bw = "nrd"`. It can also be computed using the `ks::hns()` function, or the `stats::bw.nrd()` function.

A related approach is Silverman's "rule-of-thumb" [@Silverman1986,p48], 
which uses a smaller multiplying constant, and a more robust estimate of the standard deviation:
$$
  h = 0.9 \min\left\{s, \frac{\text{IQR}}{1.34}\right\} n^{-1/5},
$$
where $\text{IQR} = \hat{Q}(0.75)-\hat{Q}(0.25)$ is the sample interquartile range. For most data sets, this will give a smaller value than @eq-nrr2. This is the default bandwidth used by `geom_density()`. It can also be computed using the `stats::bw.nrd0()` function.

Another popular bandwidth choice is the Sheather-Jones "plug-in" bandwidth [@SJ91], obtained by replacing $R(f'')$ in @eq-opth by an estimate based on the data.  Again, for most data sets, this will give a smaller value than @eq-nrr2. To use this bandwidth, set `bw = "SJ"` in `geom_density()`. It can also be computed using the `stats::bw.SJ()` function, or using  `ks::hpi()`.
...
$$
This shows that larger bandwidths are required when $f(y)/[f''(y)]^2$ is relatively large, which often occurs in the extreme tails of a distribution. So bandwidths for anomaly detection tend to be a little larger than bandwidths for other purposes. For that reason, we will use the normal reference rule @eq-nrr for most examples in this book.
