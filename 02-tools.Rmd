...
\begin{equation}
  ...
  (\#eq:kde)
\end{equation}
where $h$ is a bandwidth to be determined and $K$ is a kernel function.

To help understand this equation, let's suppose $K$ is the standard normal density function given by
$$
 K(x) = \frac{1}{\sqrt{2\pi}} e^{x^2},
$$
and let $h=20$. We'll come back to these choices later.
...
Now we will apply \@ref(eq:kde) to the `r NROW(of2021)` Old Faithful eruption durations from 2021 that are in the `oldfaithful` data set. The kernel density estimate can be visualized as a sum of kernel functions centered over each observation with width determined by $h$.

```{r kde1, fig.asp=0.1, echo=FALSE}
h <- 10
k <- tibble(x = seq(-3*h,3*h,l=100)) %>% mutate(y = dnorm(x, 0, h))
of2021kde <- of2021 %>%
  mutate(k = list(k)) %>%
  unnest(k) %>%
  mutate(x = x + duration)
of2021kde %>%
  ggplot(aes(x=x,y=y,group=eruption)) +
  geom_line(col='gray') +
  geom_rug(data = of2021, mapping=aes(x=duration, y=0), sides='b') +
...
The kernel function can be any probability density
