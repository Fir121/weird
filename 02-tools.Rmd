...
A popular way of defining anomalies is that they are observations of low probability. The kernel density estimate at each observation is (equation \@ref(eq:mkde))
$$
...
This algorithm also returns the original observation, so `k=6` has been set instead. Here the fourth closest observation has been omitted, and another nearby observation has been included instead, but otherwise the approximation has identified four of the five nearest observations.

## Local outlier factors

A popular way of using $k$-nearest neighbours for anomaly detection is via local outlier factors [@lof]. First we need some definitions.

* The $k$-distance of an observation $p$ is the distance $D_k(p)$ between observation $p$ and its $k$th nearest neighbour. That is, there are $k$ observations at a distance no more than $D_k$ from $p$. Actually, there may be more than $k$ observations within $D_k$ of $p$ if there are multiple observations at distance $D_k$ from $p$.
* The reachability distance between observations $p$ and $q$ is the

First define the "reachability distance" of an observation
