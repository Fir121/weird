...
Consequently, some algorithms use only compute the pairwise distances of the $k$ nearest observations, which reduces the problem to computing up to $kn$ distances. For some types of distances, more efficient solutions are available using kd trees [@Bentley1975;@Arya1998] that find the $k$ nearest neighbours to each observation in $O(n\log(n))$ time. 
