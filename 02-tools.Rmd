...
\begin{equation}
K(u) \ge 0, \qquad \int K(u) du = 1, \qquad \int uK(u)du = 0, \qquad\text{and}\qquad \int u^2K(u) du = 1.
  (\#eq:kernels)
\end{equation}
...
Let's suppose $K$ is the standard normal density function and let $h=15$. Then we get the following set of kernel functions.
...
$$
  \int K(\bm{u}) d\bm{u} = 1,\qquad
  \int \bm{u}K(\bm{u}) d\bm{u} = \bm{0}_d,\qquad\text{and}\qquad
