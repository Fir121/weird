...
\begin{equation}
  ...
  (\#eq:kde)
\end{equation}
where $h$ is a bandwidth to be determined and $K$ is a "kernel" function, satisfying the properties 
\begin{equation}
  K(u) \ge 0, \quad 
  \int K(u) du = 1, \quad 
  \int uK(u)du = 0, \quad\text{and}\quad 
  \int u^2K(u) du = 1.
  (\#eq:kernels)
\end{equation}
An example of such a kernel function is the standard normal density function given by $K(u) = \exp(-u^2)/\sqrt{2\pi}$. In fact, any probability density function with mean zero and variance one will be a suitable kernel function. Note that some of these properties can be relaxed [see @WJ1995], but for our purposes it is easier to work with \@ref(eq:kernels).
...
Let's suppose $K$ is the standard normal density function and let $h = `r round(h, 1)`$ (I will explain this choice below). Then we get the following set of kernel functions.
...
We made two choices when producing this estimate: the value of $h$ and the type of kernel $K$. If either was replaced with a different choice, the estimate would be different. For large data sets, it does not make much difference which kernel function is used, and it is common to use a Gaussian kernel.
...
The statistical properties of the kernel density estimator \@ref(eq:kde) have been extensively studied, and are described in several books including @WJ1995 and @Scott2015. For our purposes, the important things to note are that it is a consistent estimator of the underlying density $f$ when $h = O(n^{-1/5})$. That is, $h$ should decrease slowly as $n$ increases. The "rule-of-thumb" bandwidth \@ref(eq:ruleofthumb) satisfies this property.
...
\begin{equation}
  \hat{f}(\bm{y}) = \frac{1}{n|\bm{H}|} \sum_{i=1}^n K\left(\bm{H}^{-1}(\bm{y} - \bm{y}_i)\right),
  (\#eq:mkde)
\end{equation}
where $\bm{H}$ is a $d\times d$ bandwidth matrix and $K$ is a multivariate probability density satisfying
$$
  K(\bm{u}) \ge 0, \quad
  \int K(\bm{u}) d\bm{u} = 1,\quad
  \int \bm{u}K(\bm{u}) d\bm{u} = \bm{0}_d,\quad\text{and}\quad
  \int \bm{u}\bm{u}'K(\bm{u}) d\bm{u} = \bm{I}_d,
$$
where $\bm{0}_d$ is a $d$-vector of zeros and $\bm{I}_d$ is a $d\times d$ identity matrix.
...
We will use the `geom_density_2d()` function, which uses a bivariate Gaussian kernel with diagonal bandwidth matrix where the diagonal values are 4.7 times the size of Silverman's rule-of-thumb \@ref(eq:ruleofthumb) for univariate density estimation.
