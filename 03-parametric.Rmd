...
pspurious <- function(n, dist, c = 3, rep = 5000, ...) {
  if (length(n) == 1) {
    anomaly <- numeric(rep)
    for (j in seq(rep)) {
...
df <- expand_grid(n = n, c = seq(2, 3.5, by = 0.5)) %>%
  mutate(
    # pmax = 1-pt(c, n-1)^(2*n)
...
df %>%
  ...
  geom_label(aes(x = n * 0.9, y = 0.75, label = label, hjust = "right"), data = labels) +
  guides(col = "none") +
...
Figure \@ref(fig:pzscoretests) shows that for small sample sizes ($n<1000$), all the methods give poor results, and only Grubbs' test gives reasonable results for larger sample sizes.
...
Again, we can use simulation to compute the probability of false positives using this test, giving the results shown in Figure \@ref(fig:spuriousdixon).

```{r spuriousdixon, fig.cap = "Probability of a spurious anomaly using Dixon's test.", dependson="pspurious"}
# Function to compute probability of spurious anomaly for sample size n,
# distribution dist with parameters in ...
# rep is number of replicates
pspuriousdixon <- function(n, dist, rep = 5000, ...) {
  if (length(n) == 1) {
    pv <- numeric(rep)
    for (j in seq(rep)) {
      pv[j] <- outliers::dixon.test(dist(n, ...))$p.value
    }
    return(mean(pv < 0.05))
  } else {
    pv <- numeric(length(n))
...
As with the other tests we have considered, Dixon's test is very sensitive to the assumed data distribution making it largely useless for real data analysis.

For samples sizes $n$ in the range 3 to 30, we can use the `dixon.test()` function from the `outliers` package to perform the test. For larger sample sizes, critical values are not available for this test. So we can only use it on the last of our four examples, where it finds the outlying observation (unsurprisingly because the other data in that example are normally distributed).
